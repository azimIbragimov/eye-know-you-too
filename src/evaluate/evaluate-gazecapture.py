...
def concatenate_embeddings(df_list: Sequence[pd.DataFrame]) -> pd.DataFrame:
    """Concatenates embeddings along nb_subsequence while assigning:
       - First nb_subsequence = 0
       - Middle ones = 1
       - Last nb_subsequence = 2
    """
    
    # Use the first DataFrame as a base
    out_df = df_list[0].copy()
    ...
    # Drop columns that are not embedding-related or join keys
    drop_cols = [x for x in out_df.columns if x not in embed_cols + join_cols + ["nb_subsequence"]]
    
    # Split data into first, middle, and last groups
    first_sequences = out_df[out_df["nb_subsequence"] == out_df["nb_subsequence"].min()]
    last_sequences = out_df[out_df["nb_subsequence"] == out_df["nb_subsequence"].max()]
    middle_sequences = out_df[
        (out_df["nb_subsequence"] > out_df["nb_subsequence"].min()) &
        (out_df["nb_subsequence"] < out_df["nb_subsequence"].max())
    ]

    for i, df in enumerate(df_list[1:], start=1):
        # Separate first, middle, and last subsequences
        df_first = df[df["nb_subsequence"] == df["nb_subsequence"].min()]
        df_last = df[df["nb_subsequence"] == df["nb_subsequence"].max()]
        df_middle = df[
            (df["nb_subsequence"] > df["nb_subsequence"].min()) &
            (df["nb_subsequence"] < df["nb_subsequence"].max())
        ]
        ...
        df_first = df_first.rename(columns=cols_map).drop(columns=drop_cols)
        df_middle = df_middle.rename(columns=cols_map).drop(columns=drop_cols)
        df_last = df_last.rename(columns=cols_map).drop(columns=drop_cols)
        
        # Merge separately
        first_sequences = first_sequences.merge(df_first, how="outer", on=join_cols + ["nb_subsequence"])
        middle_sequences = middle_sequences.merge(df_middle, how="outer", on=join_cols + ["nb_subsequence"])
        last_sequences = last_sequences.merge(df_last, how="outer", on=join_cols + ["nb_subsequence"])

    # Assign new nb_subsequence values
    first_sequences["nb_subsequence"] = 0
    middle_sequences["nb_subsequence"] = 1
    last_sequences["nb_subsequence"] = 2

    # Concatenate all groups
    out_df = pd.concat([first_sequences, middle_sequences, last_sequences], ignore_index=True)
    
    return out_df

def aggregate_embeddings(df: pd.DataFrame, n: int) -> pd.DataFrame:
    group_cols = ["nb_round", "nb_subject", "nb_session", "nb_subsequence"]
    print(df.columns)
    ...
    print(centroid_embeddings.columns)
